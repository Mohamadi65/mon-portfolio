...
export default function ChronicleIndexPageClient({
  dictionary,
  locale,
}: Props) {
    ...
    <div className="min-h-screen bg-[#faf7f2] dark:bg-slate-950 flex flex-col">
      ...
      >
        ...
        <header className="mb-12 text-center">
          ...
          <h1 className="mt-2 font-serif text-4xl font-bold text-slate-900 dark:text-slate-50">
          ...
          <p className="mt-8 text-sm md:text-base leading-relaxed text-slate-700 dark:text-slate-300 text-left md:text-center">
        ...
        {/* Liste des liens */}
        <ol className="space-y-2 text-sm md:text-base text-blue-800 dark:text-blue-400">
          {links.map((item, index) => (
            <li key={item.slug} className="flex gap-2">
              <span className="w-6 text-right text-slate-700 dark:text-slate-400">
                {index + 1}.
              </span>
              <Link
