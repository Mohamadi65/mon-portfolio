...
export default function PartnersPageClient({
  ...
  const perPage = 6 // ✅ 2 colonnes × 3 lignes

  const totalPages = Math.max(1, Math.ceil(partners.length / perPage))

  const paginatedPartners = useMemo(() => {
    const start = (page - 1) * perPage
    return partners.slice(start, start + perPage)
  }, [partners, page, perPage])
                        ...
                        <div className="flex flex-wrap gap-3">
                          {phone && (
                            <a
                              ...
                              className="inline-flex items-center gap-1 rounded-full bg-pink-50 px-3 py-1 font-medium text-pink-700 hover:bg-pink-100 dark:bg-pink-900/30 dark:text-pink-300"
                            >
                          ...
                          {site && (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1 font-medium text-blue-700 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-300"
                            >
