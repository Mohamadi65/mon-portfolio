      ...
      >
        ...
        {partners.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-300">
            {t("partners.empty") || "Aucun partenaire n’a encore été enregistré."}
          </p>
        ) : (
          <section className="grid gap-6 md:grid-cols-2">
            {partners.map((p) => {
              ...
              return (
                <article
                  key={p.id}
                  className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-md dark:border-slate-800 dark:bg-slate-900"
                >
                  {/* Logo partenaire */}
                  <div className="relative h-28 w-28 flex-shrink-0 overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-800">
                    {p.logo_url ? (
                      <Image
                        src={p.logo_url}
                        alt={p.nom}
