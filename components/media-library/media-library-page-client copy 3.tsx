...
export default function MediaLibraryPageClient({
  ...
  const list = useMemo<Galerie[]>(() => { return Array.isArray(galeries) ? galeries : []; }, [galeries]);
      ...
      <main className="mx-auto flex-1 w-full max-w-6xl px-4 py-10 md:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          <div className="w-full md:w-80">
            ...
            <input
              type="search"
              className="w-full rounded-full border border-slate-300 bg-white px-4 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500"
              placeholder={
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((g) => {
              ...
              const photoCount =
                (g as any).medias?.length ??
                ((g as any).medias_count ?? 0);
